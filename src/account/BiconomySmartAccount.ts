...
import { BiconomySmartAccountConfig, Overrides, InitilizationData } from "./utils/Types";
import { UserOperation, Transaction, SmartAccountType } from "../core-types";
import NodeClient, { ISmartAccountInfo } from "../node-client";
import INodeClient from "../node-client";
import { IHybridPaymaster, BiconomyPaymaster, SponsorUserOperationDto, BiconomyTokenPaymasterRequest } from "../paymaster";
...
import { BICONOMY_FACTORY_ADDRESSES, BICONOMY_IMPLEMENTATION_ADDRESSES } from "./utils/Constants";
...
export class BiconomySmartAccount extends SmartAccount implements IBiconomySmartAccount {
  ...
  private smartAccountInfo!: ISmartAccountInfo;
  ...
  async getSmartAccountAddress(accountIndex = 0): Promise<string> {
    try {
      this.isSignerDefined();
      let smartAccountsList: ISmartAccountInfo[] = (
      ...
      smartAccountsList = smartAccountsList.filter((smartAccount: ISmartAccountInfo) => {
